...
export class SyncManager {
  ...
  private manager: RulebookManager;
   ...
   * 플랫폼에서 규칙 가져오기 (Pull)
   */
  async pullFromPlatform(platformName: string): Promise<Rule[]> {
   ...
   * 로컬과 플랫폼 간 차이점 비교
   */
  async diff(platformName: string): Promise<SyncDiffResult> {
    const adapter = getAdapter(platformName, this.basePath);
    if (!adapter) {
      throw new SyncError(`Unknown platform: ${platformName}`, { platform: platformName });
    }
    ...
    const different: { ruleId: string; differences: DiffChange[] }[] = [];
   ...
   * 플랫폼별 콘텐츠 파싱
   */
  private parsePlatformContent(platformName: string, content: string): Rule[] {
    switch (platformName) {
      case 'cursor':
   ...
   */
  private parseCursorContent(content: string): Rule[] {
    const rules: Rule[] = [];
    ...
    let currentSection: 'do' | 'dont' | 'none' = 'none';
   ...
   */
  private parseClaudeContent(content: string): Rule[] {
    const rules: Rule[] = [];
   ...
   * Claude 섹션 파싱
   */
  private parseClaudeSection(section: string): Rule | null {
    ...
    for (const line of lines) {
      ...
      if (severityMatch) {
        currentRule.severity = severityMatch[1] as 'error' | 'warning' | 'info';
      }
   ...
   * 부분 규칙을 완전한 규칙으로 변환
   */
  private completePartialRule(partial: Partial<Rule>): Rule {
   ...
   * 동기화 충돌 감지
   */
  private detectSyncConflicts(localRules: Rule[], remoteRules: Rule[]): SyncConflict[] {
    const conflicts: SyncConflict[] = [];
    const localMap = new Map(localRules.map(r => [r.id, r]));
   ...
   * 두 규칙 비교
   */
  private compareRules(a: Rule, b: Rule): DiffChange[] {
    const changes: DiffChange[] = [];
    const compareFields: (keyof Rule)[] = [
 ...
 * Diff 결과 포맷
 */
export function formatSyncDiff(diff: SyncDiffResult): string {
  const lines: string[] = [];
...
export function formatConflicts(conflicts: SyncConflict[]): string {
  ...
  const byRule = new Map<string, SyncConflict[]>();
